(() => {
  ...
  function renderEdges(){
    ...
    $('#edgeLayer').innerHTML=flow.edges.map(edge=>{
      ...
      return `<g class="edge-group ${selected.kind==='edge'&&selected.id===edge.id?'selected':''}" data-edge-id="${edge.id}"><path class="edge-hit" d="${d}"></path><path class="edge-line ${selected.kind==='edge'&&selected.id===edge.id?'selected':''}" d="${d}"></path>${edge.label?`<rect class="edge-label-bg" x="${mx-34}" y="${my-11}" width="68" height="20" rx="5"></rect><text class="edge-label" x="${mx}" y="${my+3}" text-anchor="middle">${esc(edge.label).slice(0,12)}</text>`:''}</g>`;
    }).join('');
    $$('#edgeLayer g[data-edge-id]').forEach(group=>group.onclick=e=>{e.stopPropagation();const id=group.dataset.edgeId;selected={kind:'edge',id};pendingConnection=null;renderCanvas();renderInspector();});
  }
  ...
  function bindStatic(){
    ...
    const vp=$('#canvasViewport');vp.addEventListener('wheel',e=>{e.preventDefault();zoomBy(e.deltaY<0?1.08:.92,e.clientX,e.clientY);},{passive:false});vp.onpointerdown=e=>{if(e.target.closest('.graph-node')||e.target.closest('[data-edge-id]')||e.button!==0)return;const flow=activeFlow(),start={x:e.clientX,y:e.clientY,vx:flow.view.x,vy:flow.view.y};vp.setPointerCapture(e.pointerId);vp.classList.add('panning');panState={};vp.onpointermove=ev=>{flow.view.x=start.vx+ev.clientX-start.x;flow.view.y=start.vy+ev.clientY-start.y;$('#canvasScene').style.transform=`translate(${flow.view.x}px,${flow.view.y}px) scale(${flow.view.scale})`;renderMinimap();};vp.onpointerup=()=>{vp.onpointermove=null;vp.classList.remove('panning');panState=null;saveProject();};selected={kind:'flow',id:flow.id};renderInspector();};
