(() => {
  ...
  function mount(){
    ...
    const css=document.createElement('link'); css.rel='stylesheet'; css.href='./styles-v2.css?v=3'; document.head.appendChild(css);
  ...
  function bindStatic(){
    ...
    const nodeFields=[['nodeNameInput','name'],['nodeDescriptionInput','description']];nodeFields.forEach(([id,key])=>$('#'+id).onchange=e=>{const node=activeFlow().nodes.find(n=>n.id===selected.id);if(node)commit(()=>node[key]=e.target.value);});$('#nodeTagsInput').onchange=e=>{const node=activeFlow().nodes.find(n=>n.id===selected.id);if(node)commit(()=>node.tags=e.target.value.split(',').map(x=>x.trim()).filter(Boolean));};
    ...
    $('#modalClose').onclick=$('#modalCancel').onclick=closeModal;$('#modalConfirm').onclick=()=>modalAction?.();$('#modalBackdrop').onclick=e=>{if(e.target===$('#modalBackdrop'))closeModal();};
